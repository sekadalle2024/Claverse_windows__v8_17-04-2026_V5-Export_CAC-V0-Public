"""
Test simple des corrections des problemes 1 et 3
"""
import sys
import os
...
print("=" * 80)
print("TEST CORRECTIONS PROBLEMES 1 ET 3")
print("=" * 80)
print()

# Etape 1: Charger la balance
print("Chargement de la balance de test...")
balance_file = "py_backend/P000 -BALANCE DEMO N_N-1_N-2.xls"

if not os.path.exists(balance_file):
    print(f"ERREUR: Fichier balance non trouve: {balance_file}")
    sys.exit(1)
...
try:
    ...
    print(f"OK Balance chargee: {len(balance_n)} lignes (N), {len(balance_n1)} lignes (N-1)")
...
# Etape 2: Generer les etats financiers
print("Generation des etats financiers...")
try:
    ...
    if os.path.exists(correspondances_file):
        ...
        print(f"OK Correspondances chargees")
    else:
        print(f"WARNING Fichier correspondances non trouve, utilisation d'un dict vide")
        correspondances = {}
    
    results = process_balance_to_liasse_format(balance_n, balance_n1, balance_n2, correspondances)
    print(f"OK Etats generes")
    print(f"   Cles disponibles: {list(results.keys())}")
    
    # Verifier les balances dans results
    if 'balance_n_df' in results:
        print(f"   OK balance_n_df presente")
    else:
        print(f"   WARNING balance_n_df ABSENTE - Probleme 1 ne fonctionnera pas")
    
    if 'balance_n1_df' in results:
        print(f"   OK balance_n1_df presente")
    else:
        print(f"   WARNING balance_n1_df ABSENTE - Probleme 1 ne fonctionnera pas")
    
    # Verifier le TFT
    if 'tft' in results:
        tft = results['tft']
        if isinstance(tft, dict):
            print(f"   OK TFT present (dict avec {len(tft)} cles)")
            cles_exemple = list(tft.keys())[:3]
            print(f"      Exemples de cles: {cles_exemple}")
        elif isinstance(tft, list):
            print(f"   OK TFT present (list avec {len(tft)} postes)")
        else:
            print(f"   WARNING TFT present mais format inconnu: {type(tft)}")
    else:
        print(f"   WARNING TFT ABSENT - Probleme 3 ne fonctionnera pas")
...
# Etape 3: Ajouter les balances aux results si absentes
print("Verification et ajout des balances aux results...")
if 'balance_n_df' not in results:
    print("   WARNING Ajout manuel de balance_n_df")
    results['balance_n_df'] = balance_n
if 'balance_n1_df' not in results:
    print("   WARNING Ajout manuel de balance_n1_df")
    results['balance_n1_df'] = balance_n1

print()

# Etape 4: Exporter la liasse
...
try:
    ...
    print(f"OK Export reussi: {len(file_content)} bytes")
...
# Etape 5: Verifier le contenu du fichier exporte
print("Verification du fichier exporte...")
try:
    from openpyxl import load_workbook
    
    wb = load_workbook(output_file)
    print(f"OK Fichier Excel valide")
    ...
    if 'ACTIF' in wb.sheetnames:
        ws_actif = wb['ACTIF']
        print()
        print("Verification onglet ACTIF:")
        ...
        for row in ws_actif.iter_rows(min_row=5, max_row=50, min_col=1, max_col=1):
            ...
            if cell.value == 'AI':  # Terrains
                ...
                if brut_n is not None and brut_n != 0:
                    print(f"   OK PROBLEME 1 RESOLU: Colonnes BRUT/AMORT remplies!")
                else:
                    print(f"   WARNING PROBLEME 1 PERSISTE: Colonnes BRUT/AMORT vides")
                
                break
        
        if not ref_trouve:
            print(f"   WARNING REF 'AI' non trouvee dans l'onglet ACTIF")
    ...
    if onglet_tft:
        ...
        for row in ws_tft.iter_rows(min_row=5, max_row=50, min_col=1, max_col=1):
            ...
            if cell.value == 'ZA':  # Tresorerie ouverture
                ...
                print(f"   Ligne ZA (Tresorerie ouverture) - Ligne {row_num}:")
                print(f"      Colonne I (N): {val_n}")
                print(f"      Colonne K (N-1): {val_n1}")
                
                if val_n is not None and val_n != 0:
                    print(f"   OK PROBLEME 3 RESOLU: TFT rempli!")
                else:
                    print(f"   WARNING PROBLEME 3 PERSISTE: TFT vide")
                
                break
        
        if not ref_trouve:
            print(f"   WARNING REF 'ZA' non trouvee dans l'onglet TFT")
    else:
        print(f"   WARNING Onglet TFT non trouve")
...
print(f"   Ouvrez ce fichier pour verifier manuellement:")
